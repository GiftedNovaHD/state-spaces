...
class APTx_Mish(nn.Module):
    @nn.compact
    def __call__(self,x):
        return ((1+jax.nn.tanh(x)) * (x*0.5))
...
class SequenceLayer(nn.Module):
    ...
    def __call__(self, x):
        ...
        if self.activation in ["full_glu"]:
        # Replace with APTx_Mish when shit gets fixed
            ...
            # Only apply APTx_Mish to the gate input
            x1 = self.drop(nn.gelu(x))
            x = x * jax.nn.sigmoid(self.out2(x1))
            x = self.drop(x)
        elif self.activation in ["gelu"]:
            x = self.drop(nn.gelu(x))
        elif self.activation in ["aptx_mish"]:
            x = self.drop(nn.gelu(x))
        else:
