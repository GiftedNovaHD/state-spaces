...
# New commit: 
# Add back D if everything is okay
...
# Eigenvalue handling operations
@jax.vmap
def rescale_eigs(diag_matrix):
    """Clip the eigenvalues of a diagonal matrix to be between 0 and 1.

    Args:
        matrix (np.ndarray): The diagonal matrix to clip the eigenvalues of.

    Returns:
        np.ndarray: The matrix with eigenvalues rescaled between 0 and 1.
    """
    eigenvalues = np.log(diag_matrix)
    max_value = np.max(eigenvalues)
    scale_factor = -max_value - 1e-4
    eigenvalues += scale_factor
    clipped_matrix = np.exp(eigenvalues)
    
    return clipped_matrix

@jax.vmap
def clip_eigs(diag_matrix):
    """Clip the eigenvalues of a diagonal matrix to be between 0 and 1.
    ...
    Returns:
        np.ndarray: The matrix with eigenvalues clipped between 0 and 1.
    """
    eigenvalues = np.log(diag_matrix)
    eigenvalues = np.clip(eigenvalues, -np.inf, - 1e-4)
    clipped_matrix = np.exp(eigenvalues)
...
def apply_ssm(Lambda_bar, B_bar, C_tilde, input_sequence, conj_sym, bidirectional,eig_function):
    ...
    # Eigenvalue handling
    Lambda_elements = eig_function(Lambda_elements)
    ...
    u_signal = input_sequence
    # print(f"The shape of xs {xs.shape}")
    # print(f"The shape of input_sequence: {input_sequence.shape}")
    # print(f"The shape of u_signal is {u_signal.shape}")
...
class S5SSM(nn.Module):
    ...
    """ The S5 SSM
        Args:
            ...
            eigs_handling (string): Specifies which function to use to handle eigenvalues that may lie beyond the unit circle
    """
    ...
    def setup(self):
        ...
        # Eigenvalue handling 
        if self.eigs_handling in ["rescale"]: 
            self.eig_handling_fn = rescale_eigs
        elif self.eigs_handling in ["clip"]:
            self.eig_handling_fn = clip_eigs
        else: 
            raise NotImplementedError("Eigenvalue handling function {} not implemented".format(self.eigs_handling))
    ...
    def __call__(self, input_sequence):
        ...
        ys = apply_ssm(self.Lambda_bar,
                       self.B_bar,
                       self.C_tilde, # self.D is temporarily removed
                       input_sequence,
                       self.conj_sym,
                       self.bidirectional,
                       self.eig_handling_fn)

        # Add feedthrough matrix output Du
        Du = jax.vmap(lambda u: self.D * u)(input_sequence)
        
        # (For Debugging Only) print(f"The shape of ys is: {ys.shape}")
        return ys + Du
